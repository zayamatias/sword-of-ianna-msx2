...
class IannaGame:
    ...
    def __init__(self):
        self.keyboard = [  # 0 is not pressed, 1 is pressed
            0,
            0,
            0,
            0,
        ...
        self.pause_image = pygame.image.load('artwork/pausa.png'
                ).convert()
    ...
    def pause_menu(self):
        ...
        pygame.transform.scale(self.buffer, (256 * 3, 192 * 3),
                               self.screen)
        pygame.display.flip()
        done = False
        while not done:
            ...
            for event in events:
                if event.type == pygame.locals.QUIT:
                    ...
                    if event.key == pygame.K_o:
                        ...
                        if current > 0:
                            current = current - 1
                            self.game_entities[0].current_object = \
                                current
                        self.scorearea.draw()
                        pygame.transform.scale(self.buffer, (256 * 3,
                                192 * 3), self.screen)
                        pygame.display.flip()
                    elif event.key == pygame.K_p:
                        current = self.game_entities[0].current_object
                        if current \
                            < len(self.game_entities[0].inventory) - 1:
                            current = current + 1
                            self.game_entities[0].current_object = \
                                current
                        self.scorearea.draw()
                        ...
                        if player.current_object \
                            < len(player.inventory):
                            if player.inventory[player.current_object] \
                                == 'OBJECT_HEALTH':
                                player.energy = \
                                    player.get_entity_max_energy()
                                player.inventory.remove('OBJECT_HEALTH')
                                self.scorearea.draw()
                                pygame.transform.scale(self.buffer,
                                        (256 * 3, 192 * 3), self.screen)
                                pygame.display.flip()
    ...
    def dump_menu(self):
        ...
        for i in range(0, 128):
            print ('Obj %d: %d %d' % (i, self.mymap.objects[i * 2],
                    self.mymap.objects[i * 2 + 1]))
    ...
    def export_sprites(self):
        ...
        self.export_single_sprite(self.entity_player,
                                  'export_sprites/sprite_barbaro_2.asm'
                                  , player=True)
        self.export_player_weapon(self.entity_player,
                                  'export_sprites/sprite_barbaro_1.asm'
                                  , constants.WEAPON_SWORD)
        self.export_player_weapon(self.entity_player,
                                  'export_sprites/sprite_barbaro_3.asm'
                                  , constants.WEAPON_ECLIPSE)
        self.export_player_weapon(self.entity_player,
                                  'export_sprites/sprite_barbaro_4.asm'
                                  , constants.WEAPON_AXE)
        self.export_player_weapon(self.entity_player,
                                  'export_sprites/sprite_barbaro_5.asm'
                                  , constants.WEAPON_BLADE)

        for enemy_type in [
            'OBJECT_ENEMY_SKELETON',
            'OBJECT_ENEMY_ORC',
            'OBJECT_ENEMY_MUMMY',
            'OBJECT_ENEMY_TROLL',
            'OBJECT_ENEMY_ROCK',
            ...
            entity = IannaCharacter(
                Map.enemy_spritedir[enemy_type],
                None,
                None,
                None,
                None,
            ...
            self.export_single_sprite(entity, 'export_sprites/sprite_'
                    + Map.enemy_spritedir[enemy_type] + '.asm')

        for enemy_type in ('OBJECT_ENEMY_GOLEM', 'OBJECT_ENEMY_OGRE',
                           'OBJECT_ENEMY_MINOTAUR', 'OBJECT_ENEMY_DEMON'
                           ):
            entity = IannaCharacter(
                Map.enemy_spritedir[enemy_type],
                None,
            ...
            self.export_single_sprite(entity, 'export_sprites/sprite_'
                    + Map.enemy_spritedir_export[enemy_type] + '.asm')

            entity2 = IannaCharacter(
                Map.enemy_sprite2dir[enemy_type],
                None,
                None,
                None,
                None,
            ...
            self.export_single_sprite(entity2, 'export_sprites/sprite_'
                    + Map.enemy_sprite2dir_export[enemy_type] + '.asm')

    def export_single_sprite(
        self,
        entity,
        filename,
        player=False,
    ...
    def export_menu(self):
        ...
        tileset = \
            IannaTileset(os.path.join(os.path.dirname(self.mapfile),
                         self.mymap.tilefile),
                         self.mymap.map.tilesets[0].image.height / 8,
                         self.mymap.map.tilesets[0].image.width / 8)
        try:
            os.mkdir('export')
        except (OSError, e):
            print ('Export directory already exists')

        levelscripts = ['ACTION_NONE', 'ACTION_PLAYER',
                        'ACTION_SECONDARY']
        levelstrings = []
        levelstrings_en = []
        ...
        for (kk, script) in \
            IannaScript.scripts_per_pickable_object.iteritems():
        ...
        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_SKELETON'
                ]:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_ORC']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_MUMMY']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_TROLL']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_ROCK']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_KNIGHT']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_GOLEM']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_OGRE']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_MINOTAUR'
                ]:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_DEMON']:
            if script not in levelscripts:
                levelscripts.append(script)

        for script in \
            IannaCharacter.enemy_attack_patterns['OBJECT_ENEMY_DALGURAK'
                ]:
            if script not in levelscripts:
                levelscripts.append(script)
        ...
        for y in range(0, self.mymap.map.height / 10):
            for x in range(0, self.mymap.map.width / 16):
                screen = IannaScreen(
                    self.mymap.map.layers['Fondo'],
                    self.mymap.map.layers['PrimerPlano'],
                    self.mymap.objset,
                    self.mymap.map.layers['Dureza'],
                    self.mymap.map.width,
                    self.mymap.map.height,
                    x,
                    y,
                    properties=self.mymap.map.properties,
                    )
        ...
        print ('We have', len(levelscripts), \
            'different scripts in the level')
        ...
        print ('We have', len(levelstrings), \
            'different strings in the level')

        initialcoords = self.mymap.map.properties['InitialCoords'
                ].split(',')
        mapposx = int(initialcoords[0])
        mapposy = int(initialcoords[1])
        initialscreen = self.mymap.map.properties['InitialScreen'
                ].split(',')
        mapscreenx = int(initialscreen[0])
        mapscreeny = int(initialscreen[1])

        mainfile = file('export/map.asm', 'w')
        self.mymap.tilemap.print_mapinfo(
            mainfile,
            len(levelscripts),
            len(levelstrings),
            mapposx,
...
''')
        counter = 0
        for string in levelstrings:
            f.write('string' + str(counter) + ":\t db \'" + string
                    + "\',0\n")
            counter = counter + 1
...
''')
        ...
        for y in range(0, self.mymap.map.height / 10):
            for x in range(0, self.mymap.map.width / 16):
                filenam = 'export/screen_' + str(y) + '_' + str(x) \
                    + '.asm'
...
''')
        ...
        makefile.write('map.bin: map.asm map_strings.cmp map_strings_en.cmp'
                        + screen_list + '\n')
...
''')
        ...
        for y in range(0, self.mymap.map.height / 10):
            for x in range(0, self.mymap.map.width / 16):
                ...
                makefile.write('\tpasmo ' + filenam + '.asm ' + filenam
                               + '.bin\n')
                makefile.write('\tapack ' + filenam + '.bin ' + filenam
                               + '''.cmp
...
''')
    ...
    def ProcessState(self):
        for entity in [self.game_entities[0], self.game_entities[1],
                       self.game_entities[2]]:
            if entity:
                if entity.keyboard:
    ...
    def DrawSprites(self):
        for entity in [self.game_entities[0], self.game_entities[1],
            ...
            if entity:
                try:
                    self.buffer.blit(entity.current_anim.frames[entity.anim_pos],
                            (entity.posx, entity.posy))
                except AttributeError:
                    print ('WARNING: ', entity.current_anim, \
                        'is a list, not a SevenuP object. Check!')
                    print ('Entidades: ', entity, self.game_entities[0], \
                        self.game_entities[1], self.game_entities[2])
                    print ('posx:', entity.posx, 'posy:', entity.posy, \
                        'anim_pos:', entity.anim_pos)
                if entity.extra_sprite:
                    if entity.state & 1:  # looking right
                        self.buffer.blit(entity.extra_sprite,
                                (entity.posx + 24, entity.posy))
                    else:
                        self.buffer.blit(entity.extra_sprite,
                                (entity.posx - 24, entity.posy))
    ...
    def RunScripts(self):
        for entity in self.game_entities:
            if entity:
                try:
                    if entity.state != constants.STATE_DEAD \
                        and entity.script:
    ...
    def CheckMapLimits(self):
        ...
        print ('We have', len(levelscripts), \
            'different scripts in the level, out of a maximum of 255')
        ...
        if bytecount > 1023:
            print ('WARNING!!'),
        print ('Scripts use', bytecount, \
            'bytes, out of a maximum of 1024')
        ...
        if bytecount > 1023:
            print ('WARNING!!'),
        print ('Strings in English use', bytecount, \
            'bytes, out of a maximum of 1024')
    ...
    def MainMenu(self):
        ...
        self.screen.blit(myfont.render('Select map with Q/A keys and SPACE:'
                         , False, (0, 255, 255)), (0, 0))
        selected = 0
        done = False
        ...
        while not done:
            ...
            for filename in listoftmx:
                if i == selected:
                    ...
                    color = (0, 255, 255)
                self.screen.blit(myfont.render(filename, False, color),
                                 (64, y))
                y = y + 32
                i = i + 1
            pygame.display.flip()
...
# ........    self.player_weapons = [True,False,False,False] # True if weapon is active, False if not
            ...
            self.scorearea = IannaScore(self.buffer, self.screen,
                    self.game_entities)
            self.entity_player = IannaCharacter(
                'barbaro',
                self.mymap,
                self.game_entities,
                self.scorearea,
            ...
            self.entity_player.energy = \
                self.entity_player.get_entity_max_energy()  # FIXME this is cheating!!
            self.game_entities[0] = self.entity_player
            self.mymap.load_tile_table(os.path.dirname(self.mapfile),
                    16, 16)
            self.mymap.set_screen(self.mymap.current_x,
                                  self.mymap.current_y,
                                  self.game_entities, self.scorearea)
            self.mainloop()
